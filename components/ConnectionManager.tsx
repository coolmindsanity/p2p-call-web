...
const ConnectionManager: React.FC<ConnectionManagerProps> = ({
  ...
  const renderStatusIndicator = (status: RTCPeerConnectionState) => {
    let color = 'bg-gray-400 dark:bg-gray-500'; // Default for 'new' or 'closed'
  ...
  const renderRingingContent = () => (
    <div className="flex flex-col items-center justify-center gap-4 text-center" role="status" aria-live="polite">
        <svg className="animate-pulse h-8 w-8 text-indigo-500 dark:text-indigo-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z" />
        </svg>
        <p className="text-gray-500 dark:text-gray-400">Waiting for them to answer...</p>
    </div>
  );

  return (
    <div className="w-full md:w-2/3 bg-slate-100/80 dark:bg-gray-800 rounded-lg p-6 space-y-6 flex flex-col justify-between">
      <div className="text-center">
        <h2 className="text-xl font-semibold text-indigo-600 dark:text-indigo-400">{renderTitle()}</h2>
        {renderStatusIndicator(connectionState)}
      </div>
      
      {callState === CallState.WAITING_FOR_ANSWER && callId && (
        <div className="space-y-4 text-center">
          <p className="text-gray-600 dark:text-gray-300">Share this Call ID with your peer:</p>
          <div className="p-4 bg-slate-200 dark:bg-gray-900 rounded-lg inline-block">
            <p className="font-mono text-2xl tracking-widest text-teal-600 dark:text-teal-300">{callId}</p>
          </div>
          <button
          ...
          >
           ...
           <p className="text-sm text-gray-500 dark:text-gray-400 pt-4">Waiting for peer to join...</p>
        </div>
      )}
      ...
      {(callState === CallState.JOINING || callState === CallState.CREATING_ANSWER) && (
        <div className="flex flex-col items-center justify-center gap-4 text-center" role="status" aria-live="polite">
          ...
          <p className="text-gray-500 dark:text-gray-400">Connecting to peer...</p>
        </div>
      )}
